...
| **Joint number** | Weld Planning's `jointNumber`: 2-letter prefix, hyphen, 5 digits, e.g. `ST-00001` (seed: `ST` structural, `PI` pipe). Labeled **"System"** in the UI (create/edit form, detail page, Advanced Search, mass-edit) as of 2026-09-23 — the field itself sits right next to the separate `joint` field (also now system-joint format, see below), and "Joint Number"/"Joint" side by side read as near-duplicates. The `jointNumber` property name is unchanged, only the display label. |

...
- **Repair now routes somewhere specific on signoff, instead of just falling through to whatever's next** (`SignoffService.signStage()`, 2026-09-23) — checked in this priority order: **Allowable thickness exceeded** (checkbox) → reopens that phase's NDT UT/RT stage (`${phase}-ndt-utrt`); else **Grind Only** → reopens that phase's NDT VT/5X stage (`${phase}-ndt-vt5x` — always VT/5X, "the applicable VT signoff", regardless of which method actually failed, e.g. a rejected Final MT routes to Final VT); else **Weld Repair** → inserts a new **Excavation NDT** stage (`EXCAVATION_NDT_STAGE` in `workflow.ts`: the same common NDT fields every NDT stage shares — Procedure Used, Probationary Inspector, Partial — no method choice of its own); **Cut**, or no Repair Code chosen, has no special routing. "Which phase" (root/layer/final), the exact rejecting stage id, and its inspection method are captured on the Repair stage's own `inputs['originPhase']`/`['originStageId']`/`['originInspectionType']` when it's created (parsed off the rejecting NDT stage, e.g. `root-ndt-mtpt` + `inspectionType: 'mt'`) — internal bookkeeping, not real `StageField`s, so none of it is rendered as a field. `stageFromTemplate()` (new, `workflow.ts`) builds a live `WorkflowStage` from a `StageTemplate` for both Repair and Excavation NDT, replacing a hand-written object literal that used to exist only for Repair.
- **Excavation NDT's own signoff routes onward too** (2026-09-23; "all weld repairs require the original joint inspection unless otherwise stated") — UNSAT routes back to Repair like any other NDT reject, via its `rejectToStage: 'repair'` and the same generic reject-to-stage reopen logic every other stage already uses. **SAT** reopens "the original joint inspection" — the exact NDT stage (`inputs['originStageId']`) that rejected the joint in the first place, read off the still-present (signed) Repair stage — **unless** that original inspection was **PT** and the job's material (Material Type 1 **or** 2 — either counts) is **non-ferrous or austenitic**, in which case it reopens that phase's VT/5X stage instead ("require 5X instead of PT"). Non-ferrous/austenitic is looked up per material code from a new admin-managed table (`data/material-classification.ts`/`isNonFerrousOrAustenitic()`, Admin > Material Classification, `admin-material-classification/` — same load/save/add/remove shape as `data/mcl-traceability.ts`/Material Traceability) rather than hardcoded, since the user explicitly didn't want the metallurgy classification guessed in code. **Seed data is an unreviewed best guess** (`DEFAULT_ENTRIES` in `material-classification.ts`): SS-304/SS-316 (austenitic stainless), TI-6400/AL-1010/30-CUNI/60-INC (non-ferrous) → true; 02CS/40-AS/11CI (ferrous) and DS-2205 (duplex stainless — mixed austenitic/ferritic, not purely one or the other) → false; correct via the admin table, not a code change. **Demo aid**: same live info banner pattern as Repair (`JointPageComponent.repairRouteLabel()`, now handles both stage ids) shown on the Excavation NDT view.

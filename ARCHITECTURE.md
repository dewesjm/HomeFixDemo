...
| Term | Meaning |
|---|---|
| **Hull** | The record a job belongs to (`job.hull`, letter + 4 digits, e.g. `K7234`). **Not unique** — many jobs share a hull. Replaces the old "Project" / job number / title. There is no job `title` and no joint `title` either. |
...
src/app/
  ...
  weld-record/            All Weld Record (EWR) screens + their admin pages, grouped under one folder (moved
    ...
    makeup/                Makeup — acting-foreman grants, flat grid (see Signoff panel section)
    ...
    admin/
      ...
      admin-weld-positions/, admin-joint-designs/, admin-banner/, admin-teams/, admin-quick-links/   Other admin pages
...
- **Joint details** — 3 columns (stack on mobile): job info (XREFID, **Ship** — 3-digit, added 2026-09-23, sits right above Hull —, Hull, ...), joining/join-to (MCL 1/2, Material Type 1/2, Joining/Join To Item — 1 letter + 8 digits + hyphen + 2 digits, e.g. `S12341001-14` (user-specified format, 2026-09-23, replacing the earlier invented piece-mark style like `HPF-D120-1`; leading letter still varies, still invented, not a real area-code scheme), NDT data (RT/NDT/UT/VT as `X` or `5X` — `ndtLabel()` derives these purely from `job.ndt`'s free text, it does not read `job.rtRoot`/`rtFinal` themselves, which since 2026-09-23 hold the *degree* required for RT, not an X/5X marker — see "Degree of RT Performed" under Signoff panel); "Show more" reveals additional data and attribute codes. Attribute codes show as `code description` (`attrCodeDisplay()`, looked up via `characteristicLabel()` in `data/characteristics.ts`, the same table Admin > Attribute Codes manages) — fixed 2026-09-22: `job.attributeCode1-4`'s seed pool used to be unrelated 2-letter codes (`AB`, `CD`, …) that never matched `CHARACTERISTIC_CODES`' real codes, so descriptions could never resolve; `ATTR_CODES` in `jobs.ts` now reuses `CHARACTERISTIC_CODES`' own codes.
...
- **NDT routing** — each phase's NDT follows its own **Joint Details** value (2026-09-24, `ndtRequirement()`/`phaseNdtRequirements()` in `workflow.ts`): **NDT Root** → Root, **NDT Each** → Layer, **NDT Final** → Final. The general `job.ndt` field no longer drives routing. Valid values for all three: blank, 5X, MT, MT/PT, NA, PT, UT, VT (`NDT_REQUIREMENT_VALUES`, `jobs.ts`). Blank or NA means no NDT for that phase (e.g. Final Layer goes straight on to Final Weld). MT/PT gives the MT/PT stage with a choice; any other value gives that method's stage with its Type **locked** to it (e.g. MT → MT/PT stage locked to MT; UT → UT/RT stage locked to UT). So each phase has at most one NDT stage. A single-option Type on an inspection stage is pre-filled, disabled, and explained under the droplist (`SignoffPanelComponent.typeLockNote()`, also used for Excavation NDT). Root's "Did you perform 5X" question only shows when NDT Root allows 5X. **RT Root / RT Final** valid values: blank, 10, 100, 360, 60, 75, NA (the required Degree of RT, checked when an RT inspection is signed). Joint Details shows all five values as stored. CURRENT_VERSION 2.10.7.

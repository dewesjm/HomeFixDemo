...
src/app/
  ...
  weld-record/            All Weld Record (EWR) screens + their admin pages, grouped under one folder (moved
    ...
    admin/
      ...
      admin-makeup/           Other admin pages
...
- **Admin > Makeup** (`admin-makeup/`, `data/makeup.ts`, 2026-09-23) — a Foreman granted temporary "makeup" (acting-foreman) status, and the ad hoc team they build under it while it's active: `MakeupGrant { foremanId, startDate, endDate, members: { personId, role }[] }`, list + click-into-detail like Admin > Teams. **UI-only, explicitly not wired to anything**: the app has no concept of "who's logged in as which specific person" (only a role *filter*, e.g. Pipe Search's `selectedRole`), and there's no time-boxed AD group membership at this org to hang real enforcement off of — so this is set up and reviewable but doesn't grant any actual access, same demo-aid honesty as Admin > Teams' permission matrix (also unenforced, mocking what AD groups would really do). Foreman choices come from `PEOPLE` filtered by `title === 'Foreman'` (identity, AD-sourced in the real system) — deliberately not the app's own `Role` type, which is per-stage routing, not who a person is. Member roles exclude Foreman (that's the grant itself) and View.

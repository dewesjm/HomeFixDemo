...
- **Fit-Up Insp** — fixed, non-interactive **Type: Fit-Up Inspection** / **Layer: Fit** fields at the top (added 2026-09-23, same visual convention as the Type dropdown other inspector stages use, but no options — there's only ever one value for each here), then the verification grid against fabrication data, Release-to-welding checkbox. Pressing Signoff without checking every verification box highlights the unchecked row/checkbox red (`table-error`/`text-error`/`checkbox-error`) instead of only the one generic "Verify every fitting value" reason — `validateStageFields()` (joint-page) sets a per-field error for each unchecked box, same `ctx().fieldError()` mechanism every other required field already uses; `ctx().clearFieldError()` clears a row's highlight the moment it's checked. Added 2026-09-23.

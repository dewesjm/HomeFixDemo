...
- **NDT routing** — `job.ndt` is regex-matched: `hasUTorRT`, `hasMTorPT`, `hasVT` for **Root and Final** only. **Layer NDT follows NDT Each (`job.ndtEach`) alone** (2026-09-24, `layerNdtRequirement()` in `workflow.ts`): valid values blank, 5X, MT, MT/PT, NA, PT, UT, VT (`NDT_EACH_VALUES`, `jobs.ts`); blank or NA means no Layer NDT, so Final Layer goes straight on to Final Weld; MT/PT gives the MT/PT stage with a choice; any other value gives that method's stage with its Type **locked** to it (e.g. MT → MT/PT stage locked to MT; UT → UT/RT stage locked to UT). A single-option Type on an inspection stage is pre-filled, disabled, and explained under the droplist (`SignoffPanelComponent.typeLockNote()`, also used for Excavation NDT). NDT Root / NDT Final are meant to drive Root/Final the same way eventually; not built yet (user: only Layer changes for now). CURRENT_VERSION 2.10.6.
